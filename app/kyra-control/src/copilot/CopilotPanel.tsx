...
export function CopilotPanel(){
  const { msgs, push } = useChat([
    { role:'assistant', text: "Hi! I can check status, run once, start/stop, show logs, list PRs, or claim the next item. Say 'run once' to try." }
  ])
  ...
  async function runIntent(text:string){
    ...
    const exec = async (fn:()=>Promise<any>, label:string)=>{
      setConfirm({ label, do: async ()=>{
        ...
        try{
          const out = await fn()
          push({ role:'assistant', text: (out.stdout||out.stderr||"Done.").slice(0,8000) })
        }catch(e:any){
    ...
    switch(intent.type){
      ...
      case 'HELP':   return push({ role:'assistant', text: "I can: status, run once, start, stop, tail logs, list PRs, claim next." })
      case 'LLM': {
        setBusy(true)
        try{
          const sys = "You are Kyra Build Copilot. Concise. Confirm before actions."
  ...
  return (
    <div style={{background:'#111', borderRadius:8, padding:12}}>
      ...
      <form onSubmit={onSend} style={{display:'flex', gap:8, marginTop:8}}>
        <input name="msg" placeholder="status / run once / show logs / list PRs ..." style={{flex:1}} />
